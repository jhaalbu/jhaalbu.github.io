...
const elevationProfile = new ElevationProfile({
  view,
  profiles: [
    new ElevationProfileLineInput({ color: [245, 203, 66], title: "Hodlekve rundt halvdistanse" }),
  ],
  visibleElements: {
    selectButton: false,
    sketchButton: false,
...
(async () => {
  ...
  const response = await fetch("./hodlekve_rundt_kort.gpx");
 ...
 const kambaFjell = {
  ...
  const pointsLayer = new FeatureLayer({
    source: [startPoint, endPoint, helleBerg],
